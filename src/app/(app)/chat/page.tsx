...
// Resolves DM display name — prefers the live userMap over the stale
// participantNames stored in the channel document.
...
// Bigger picker emoji list (used in MessageInput)
...
const EmojiPicker = memo(function EmojiPicker({
  ...
  return (
    <div
      ref={ref}
      className="absolute z-50 bottom-full mb-1 right-0 flex gap-0.5 bg-white border border-gray-200 rounded-xl shadow-lg px-2 py-1.5"
    >
      {QUICK_EMOJIS.map(e => (
        <button
          key={e}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {isCode && !message.isLongText ? (
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 my-1 max-w-xl">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs bg-[#1B2B5E]/10 text-[#1B2B5E] px-2 py-0.5 rounded font-medium">
                Long message
              </span>
        ...
        {message.attachments?.map(att => (
          <div
            key={att.id}
            className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 mt-1.5 max-w-xs"
          >
            ...
            <a
              ...
              className="text-[#1B2B5E] text-xs font-medium hover:underline"
            >
              Open
            </a>
          </div>
...
// ── Message group (groups consecutive same-sender messages) ───────────────────
      ...
      <div className={`border rounded-xl overflow-hidden transition-colors bg-white ${
        ...
        <div className="flex items-center justify-between px-2 pb-2 pt-0.5">
            ...
            <div className="relative" ref={emojiRef}>
              <button
                ...
                className={`p-1.5 rounded-md hover:bg-gray-100 transition-colors ${
                  showEmojiPicker ? 'text-[#1B2B5E] bg-[#1B2B5E]/8' : 'text-gray-400 hover:text-gray-700'
                }`}
                title="Emoji"
              >
                <Smile size={14} />
          ...
          <div className="flex items-center gap-2">
            {text.length > 500 && (
              <span className="text-[10px] text-gray-400">{text.length.toLocaleString()}</span>
            )}
            <button
              onClick={doSend}
              ...
              className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
                isEmpty || sending
                  ...
                  : 'bg-[#1B2B5E] text-white hover:bg-[#2D4080] active:scale-95'
              }`}
            >
            ...
            <div className="space-y-0.5">
              {publicChannels.map(ch => (
                <ChannelItem
                  ...
                  currentUserId={currentUserId}
                  unread={(unreadByChannel[ch.id] ?? 0) > 0}
                  userMap={userMap}
                  photoMap={photoMap}
...
// ── Conversation (main message area) ─────────────────────────────────────────
...
function Conversation({
  ...
  useEffect(() => {
    setMessages([]);
    // Mark the channel read immediately when it opens
    ...
    const unsub = chatService.subscribeToChannel(channel.id, msgs => {
      ...
      // Mark read again whenever new messages arrive — so the badge never re-appears
      // while the user is actively viewing this channel
      if (currentUser) markChannelRead(currentUser.uid, channel.id);
    });
    return unsub;
          ...
          dmHeaderPhoto
            ...
            : (
              <div className="w-8 h-8 rounded-full bg-[#1B2B5E]/10 flex items-center justify-center flex-shrink-0">
                <span className="text-[#1B2B5E] text-xs font-bold">{getDMInitials(channel, currentUserId, userMap)}</span>
              </div>
            )
        ) : (
            ...
            <EmptyState
              ...
              description={
                isDM
                  ...
                  : channel.description ?? 'Be the first to start the conversation!'
              }
            />
          </div>
        ) : (
...
// ── Chat page inner (uses useSearchParams) ────────────────────────────────────
...
function ChatPageInner() {
  ...
  // Live maps of uid → name and uid → photoURL so DM names/avatars never go stale
  ...
  // Single subscription builds both maps; fires on any profile change
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Real-time channel subscription keeps the sidebar always up-to-date
    const unsub = chatService.subscribeToChannels(allChannels => {
      ...
      if (!initialSelectDone.current) {
        ...
        if (dmParam) {
          ...
          // DM channel might not be in accessible list — fetch single doc
          if (!accessible.find(c => c.id === dmParam)) {
            chatService.getChannelById(dmParam).then(dmCh => {
              if (dmCh) {
                setChannels(prev =>
                  prev.some(c => c.id === dmParam) ? prev : [...prev, dmCh]
                );
              }
            });
          }
        ...
        // Default: select first public channel
    ...
    <div className="flex h-full overflow-hidden">
      ...
      {activeChannel ? (
        ...
        <div className="hidden md:flex flex-1 items-center justify-center bg-white">
          {loaded ? (
            <EmptyState
              icon={<MessageSquare size={28} className="text-gray-300" />}
              title="Select a channel"
              description="Choose a channel from the left to start chatting."
            />
          ) : (
            <div className="flex flex-col items-center gap-3">
              <div className="w-8 h-8 border-2 border-[#1B2B5E]/20 border-t-[#1B2B5E] rounded-full animate-spin" />
